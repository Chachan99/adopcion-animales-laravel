        ...
        <div class="bg-white shadow-xl rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-6 sm:p-8">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="pt-4 border-t border-gray-200 mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <form action="{{ route('animales-perdidos.destroy', $mascota->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta publicación? Esta acción no se puede deshacer.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center text-sm font-medium text-red-600 hover:text-red-500 transition-colors duration-200">
                                <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
...
@if($mascota->latitud && $mascota->longitud)
    ...
    <script>
        function initMap() {
            const location = { lat: {{ $mascota->latitud }}, lng: {{ $mascota->longitud }} };
